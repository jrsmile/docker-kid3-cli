...
RUN DEBIAN_FRONTEND=noninteractive apt-get -qq update -y && DEBIAN_FRONTEND=noninteractive apt-get -oDpkg::Use-Pty=false -o DPkg::options::="--force-confdef" --no-install-recommends -qq install software-properties-common -y && add-apt-repository ppa:ufleisch/kid3
